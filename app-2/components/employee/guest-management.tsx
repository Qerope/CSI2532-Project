  ...
  const handleAddGuest = async () => {
    ...
    try {
      const response = await fetch("http://localhost:5010/api/guest", {
        ...
        body: JSON.stringify({
          ...newGuest,
          date_of_checkin: format(new Date(newGuest.date_of_checkin), "yyyy-MM-dd"),
        }),
      })
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className="col-span-3 justify-start text-left font-normal">
                      ...
                      {newGuest.date_of_checkin ? format(new Date(newGuest.date_of_checkin), "PPP") : "Select date"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0">
                    <Calendar
                      mode="single"
                      selected={newGuest.date_of_checkin ? new Date(newGuest.date_of_checkin) : undefined}
