...
export function AvailabilityView() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const url = "http://localhost:5010/api/views/availability?"
                  ...
                  {availabilityData.map((item, index) => (
                    <Card key={index}>
                      <CardContent className="p-6">
                        <div className="text-2xl font-bold">{item.available_rooms}</div>
                        <div className="text-sm text-muted-foreground">Available rooms in {item.area}</div>
                      </CardContent>
                    </Card>
                  ))}
                </div>
